...
QImage ImageUtils::filledImage(uint width, uint height, const QColor &colour)
{
    ...
    return filledImage(QSize(width, height));
}
    ...
    */
    ...
    for (int layerIndex = 0; layerIndex < images.size(); ++layerIndex) {
        ...
        for (int cellIndex = 0; cellIndex < smallerCellCount; ++cellIndex) {
            const int oldColumn = layerIndex % oldColumnCount;
            const int oldRow = layerIndex / oldColumnCount;
            ...
            const int newColumn = layerIndex % columns;
            const int newRow = layerIndex / columns;
