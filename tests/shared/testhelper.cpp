...
QByteArray TestHelper::activeFocusFailureMessage(const QObject *object, const QString &when)
{
    const QString whenText = !when.isEmpty() ? ' ' + when : QString();
    return QString::fromLatin1("Expected object %1 to have focus%2, but %3 has it instead")
            .arg(detailedObjectName(object)).arg(whenText).arg(detailedObjectName(window->activeFocusItem())).toLatin1();
}
...
QString TestHelper::detailedObjectName(const QObject *object)
{
    if (!object)
    ...
    if (!parent) {
        const auto asItem = qobject_cast<const QQuickItem*>(object);
        if (asItem)
...
bool TestHelper::fuzzyColourCompare(const QColor &actualColour, const QColor &expectedColour, int fuzz, QString &failureMessage)
{
    const int rDiff = qAbs(expectedColour.red() - actualColour.red());
    if (rDiff > fuzz) {
        failureMessage = fuzzyColourCompareFailMsg(actualColour, expectedColour, QLatin1String("red"), rDiff, fuzz);
        return false;
    }

    const int gDiff = qAbs(expectedColour.green() - actualColour.green());
    if (gDiff > fuzz) {
        failureMessage = fuzzyColourCompareFailMsg(actualColour, expectedColour, QLatin1String("green"), gDiff, fuzz);
        return false;
    }

    const int bDiff = qAbs(expectedColour.blue() - actualColour.blue());
    if (bDiff > fuzz) {
        failureMessage = fuzzyColourCompareFailMsg(actualColour, expectedColour, QLatin1String("blue"), bDiff, fuzz);
        return false;
    }

    const int aDiff = qAbs(expectedColour.alpha() - actualColour.alpha());
    if (aDiff > fuzz) {
        failureMessage = fuzzyColourCompareFailMsg(actualColour, expectedColour, QLatin1String("alpha"), aDiff, fuzz);
        return false;
    }
...
void TestHelper::fuzzyImageCompare(const QImage &actualImage, const QImage &expectedImage, int fuzz, const QString &context)
{
    ...
    if (actualImage.size() != expectedImage.size()) {
        ...
        saveImagesToPwd();
        QFAIL(qPrintable(QString::fromLatin1("Failure comparing images %1: actual size %2 is not the same as expected size %3)")
    ...
    for (int y = 0; y < actualImage.height(); ++y) {
        for (int x = 0; x < actualImage.width(); ++x) {
            ...
            if (!fuzzyColourCompare(actualImage.pixelColor(x, y), expectedImage.pixelColor(x, y), fuzz, failureMessage)) {
                // TODO: remove fromLatin1 call for third arg in Qt 6; only added it to workaround ambiguous argument warning
                const QString contextStr = context.isEmpty() ? context : " (" + context + ')';
                saveImagesToPwd();
                QFAIL(qPrintable(QString::fromLatin1("Failure comparing pixels%1 at (%2, %3):%4")
                    .arg(contextStr).arg(x).arg(y).arg(failureMessage).toLatin1()));
            }
        }
    }
}

void TestHelper::compareImages(const QImage &actualImage, const QImage &expectedImage, const QString &context)
{
    QTEST_CHECKED(fuzzyImageCompare(actualImage, expectedImage, 0, context));
}
...
void TestHelper::clickButton(QQuickItem *button, Qt::MouseButton mouseButton)
{
    QTEST_CHECKED(verifyItemVisible(button));
    ...
    QVERIFY2(buttonClickedSpy.count() == 1, qPrintable(QString::fromLatin1(
...
void TestHelper::verifyItemVisible(QQuickItem *item, EnsureVisibleFlags flags)
{
    ...
    if (flags.testFlag(CheckOpacity)) {
        QVERIFY2(item->opacity() > 0.0, qPrintable(
...
//            VERIFY(delegateItem);
//            if (!verifyItemVisible(delegateItem, CheckAllVisibleProperties))
//                return false;
//        }
//    }
//    return true;
...
{
    QTEST_CHECKED(triggerShortcut("newShortcut", app.settings()->newShortcut()));
}
...
void TestHelper::createNewProject(Project::Type projectType, const QVariantMap &args)
{
    ...
    QTEST_CHECKED(triggerNewProject());
    ...
    QVERIFY(newProjectButton);
    QTEST_CHECKED(clickButton(newProjectButton));
    QVERIFY(newProjectButton->property("checked").toBool());
    // The new project popup should be hidden, and now a project-specific project creation popup should be visible.
    QTRY_VERIFY(!newProjectPopup->property("visible").toBool());
    ...
    if (projectType == Project::TilesetType) {
        ...
        QTEST_CHECKED(setupTempTilesetProjectDir());

        // Now the New Tileset Project popup should be visible.
        QTRY_VERIFY(findOpenPopupFromTypeName("NewTilesetProjectPopup"));
        const QObject *newTilesetProjectPopup = findOpenPopupFromTypeName("NewTilesetProjectPopup");
        QTRY_VERIFY2(newTilesetProjectPopup->property("opened").toBool(),
        ...
        if (tilesetTilesWide == -1 || tilesetTilesHigh == -1) {
            ...
            QVERIFY(tilesWideSpinBox->property("value").toInt() == tilesetTilesWide);

            QVERIFY(tilesHighSpinBox->setProperty("value", tilesetTilesHigh));
            QVERIFY(tilesHighSpinBox->property("value").toInt() == tilesetTilesHigh);
            ...
            QVERIFY(transparentBackgroundCheckBox->property("checked").toBool());

            if (!transparentTilesetBackground) {
                QTEST_CHECKED(clickButton(transparentBackgroundCheckBox));
                QVERIFY(!transparentBackgroundCheckBox->property("checked").toBool());
            }
        }
        ...
        QVERIFY(okButton);
        QTEST_CHECKED(clickButton(okButton));
        QTRY_VERIFY2(!newTilesetProjectPopup->property("visible").toBool(),
        ...
        if (projectType == Project::LayeredImageType) {
            ...
            QTEST_CHECKED(setupTempLayeredImageProjectDir());
        }

        // Now the New [Layered] Image Project popup should be visible.
        QObject *newImageProjectPopup;
        QTEST_CHECKED(ensureNewImageProjectPopupVisible(projectType, &newImageProjectPopup));
        ...
        QVERIFY2(imageWidthSpinBox->property("value").toInt() == expectedImageWidth, qPrintable(QString::fromLatin1(
        ...
        QVERIFY2(imageHeightSpinBox->property("value").toInt() == expectedImageHeight, qPrintable(QString::fromLatin1(
        ...
        QVERIFY(imageWidthSpinBox->property("value").toInt() == imageWidth);

        QVERIFY(imageHeightSpinBox->setProperty("value", imageHeight));
        QVERIFY(imageHeightSpinBox->property("value").toInt() == imageHeight);
        ...
        QVERIFY(!transparentImageBackgroundCheckBox->property("checked").toBool());

        if (transparentImageBackground) {
            QTEST_CHECKED(clickButton(transparentImageBackgroundCheckBox));
            QVERIFY(transparentImageBackgroundCheckBox->property("checked").toBool() == transparentImageBackground);
        }
        ...
        QVERIFY(okButtonClickedSpy.isValid());
        ...
        QVERIFY(cancelButtonClickedSpy.isValid());
    ...
    QTEST_CHECKED(updateVariables(true, projectType));
}
...
void TestHelper::ensureNewImageProjectPopupVisible(Project::Type projectType, QObject **popup)
{
    ...
    QTRY_VERIFY(findOpenPopupFromTypeName(newProjectPopupTypeName));
    QObject *newImageProjectPopup = findOpenPopupFromTypeName(newProjectPopupTypeName);
    QVERIFY(newImageProjectPopup);
...
void TestHelper::updateVariables(bool isNewProject, Project::Type projectType)
{
    ...
    QTRY_VERIFY2(!project, qPrintable(QString::fromLatin1("The last project (%1) should have been destroyed by now")
        .arg(project ? project->url().toString() : "WHOOPS")));
    ...
    QVERIFY(splitScreenToolButton->isEnabled() == true);
    QVERIFY(splitScreenToolButton->property("checked").toBool() == canvas->isSplitScreen());
    QVERIFY(lockSplitterToolButton->isEnabled() == canvas->isSplitScreen());
    QVERIFY(lockSplitterToolButton->property("checked").toBool() == !canvas->splitter()->isEnabled());
    ...
    QVERIFY(penForegroundColourButton->setProperty("checked", QVariant(true)));
...
//    TRY_VERIFY(canvas->hasActiveFocus());
    ...
    if (isNewProject) {
        QVERIFY(project->url() == QUrl());
        QVERIFY(project->isNewProject());
        QVERIFY(project->creationVersion() == QVersionNumber::fromString(qApp->applicationVersion()));
    ...
    // Sanity check.
    QTRY_VERIFY(canvas->window() == canvasSizeToolButton->window());
    QVERIFY(!canvas->splitter()->isPressed());
    QVERIFY(!canvas->splitter()->isHovered());

    if (isNewProject) {
        QVERIFY(canvas->splitter()->position() == 0.5);
    }
    ...
    if (projectType == Project::TilesetType) {
        ...
        QVERIFY(tilesetSwatchPanel);
        QVERIFY(tilesetSwatchPanel->isVisible() == true);
        QVERIFY(!qFuzzyIsNull(tilesetSwatchPanel->width()));
        // This started failing recently for some reason, but giving it some time seems to help.
        QTRY_VERIFY(!qFuzzyIsNull(tilesetSwatchPanel->height()));
        ...
        QVERIFY(tilesetSwatchFlickable->property("contentY").toReal() == 0.0);

        QVERIFY(imageGrabber.requestImage(tileCanvas));
        QTRY_VERIFY(imageGrabber.isReady());
        QVERIFY(imageGrabber.takeImage().pixelColor(16, 16) == tileCanvas->mapBackgroundColour());
    } else {
    ...
    QTEST_CHECKED(collapseAllPanels());
    ...
    if (isNewProject) {
        ...
        QVERIFY(colourPanelFlickable->setProperty("contentY", 0));
    }
}

void TestHelper::saveChanges(const QString &expectedErrorMessage)
{
    if (project->isNewProject()) {
        QFAIL("Cannot save changes because the project is new, " \
             "and we cannot automate interaction with native Save As dialog");
    }

    if (!project->hasUnsavedChanges())
        QFAIL("Cannot save changes because there are none");

    const QObject *saveChangesDialog = findOpenPopupFromTypeName("SaveChangesDialog");
    QVERIFY(saveChangesDialog);
    QTRY_VERIFY(saveChangesDialog->property("opened").toBool());

    QQuickItem *saveChangesButton = findDialogButtonFromObjectName(saveChangesDialog, "saveChangesDialogButton");
    QVERIFY(saveChangesButton);
    QTEST_CHECKED(clickButton(saveChangesButton));
    QTRY_VERIFY(!saveChangesDialog->property("visible").toBool());

    if (!expectedErrorMessage.isEmpty()) {
        // The save should fail.
        QTEST_CHECKED(verifyErrorAndDismiss(expectedErrorMessage));
        return;
    }
...
{
    if (!project->hasUnsavedChanges())
        QFAIL("Cannot discard changes because there are none");
    ...
    QVERIFY(discardChangesButton);
    QTEST_CHECKED(clickButton(discardChangesButton));
    QTRY_VERIFY(!saveChangesDialog->property("visible").toBool());
}
...
void TestHelper::verifyErrorAndDismiss(const QString &expectedErrorMessage)
{
    ...
    QVERIFY(errorDialog);
    QTRY_VERIFY(errorDialog->property("opened").toBool());
    ...
    QVERIFY2(errorMessage.contains(expectedErrorMessage), qPrintable(QString::fromLatin1(
...
{
    QVERIFY2(tempProjectDir, qPrintable(QString::fromLatin1(
...
void TestHelper::setupTempTilesetProjectDir()
{
    ...
    QTEST_CHECKED(setupTempProjectDir(toCopy, &copiedPaths));
...
{
    QTEST_CHECKED(setupTempProjectDir());
}
...
void TestHelper::setupTempProjectDir(const QStringList &resourceFilesToCopy, QStringList *filesCopied)
{
    ...
    foreach (const QString &baseName, resourceFilesToCopy) {
        QTEST_CHECKED(copyFileFromResourcesToTempProjectDir(baseName));
...
void TestHelper::collapseAllPanels()
{
    if (project->type() == Project::TilesetType)
        QTEST_CHECKED(togglePanel("tilesetSwatchPanel", false));
    else if (project->type() == Project::LayeredImageType)
        QTEST_CHECKED(togglePanel("layerPanel", false));

    QTEST_CHECKED(togglePanel("colourPanel", false));

    QTEST_CHECKED(togglePanel("swatchesPanel", false));
    ...
    if (project->type() == Project::ImageType || project->type() == Project::LayeredImageType) {
        ...
        if (animationPanel->isVisible())
            QTEST_CHECKED(togglePanel("animationPanel", false));
    }
}
...
void TestHelper::togglePanel(const QString &panelObjectName, bool expanded)
{
    ...
    QTEST_CHECKED(ensurePanelPolished(panel));

    const qreal originalHeight = panel->height();
    QVERIFY(panel->setProperty("expanded", QVariant(expanded)));
    QVERIFY(panel->property("expanded").toBool() == expanded);
    ...
    if (expanded) {
        ...
        QTRY_VERIFY2(panel->height() > originalHeight, qPrintable(QString::fromLatin1(
        ...
        if (panel->isVisible()) {
            ...
            QTRY_VERIFY2(panel->height() < 100, qPrintable(QString::fromLatin1(
...
void TestHelper::ensurePanelPolished(QQuickItem *panel)
{
    QVERIFY(panel);

    auto panelContentItem = panel->property("contentItem").value<QQuickItem*>();
    QVERIFY(panelContentItem);
    if (QQuickTest::qIsPolishScheduled(panelContentItem))
        QVERIFY(QQuickTest::qWaitForPolish(panelContentItem));
}
...
void TestHelper::switchMode(TileCanvas::Mode mode)
{
    ...
    QTEST_CHECKED(clickButton(modeToolButton));
